...
# ---------------------------------------------------------------------
# MCP Tool: Refresh schema cache
# ---------------------------------------------------------------------
@mcp.tool(name="refresh_schema_cache", description="Reloads the table and column cache from HANA")
...
class DataAnalyzer:
    ...
    @staticmethod
    def analyze_aggregation(results: List[Dict], group_by: str, aggregations: Dict) -> QueryInsight:
        if not results:
            return QueryInsight(
                summary=f"No data for aggregation by {group_by}",
                key_findings=["No groups found for aggregation"],
                data_quality_notes=["Check if group_by column has valid data"],
                recommendations=["Verify the group_by column and try different aggregations"],
                raw_data=results,
            )

        findings = [f"Generated {len(results)} groups by '{group_by}'"]
        recommendations = []

        # Analyze aggregation results
        for agg_col, agg_func in aggregations.items():
            agg_key = f"{agg_func.lower()}_{agg_col}"
            values = [row.get(agg_key) for row in results if row.get(agg_key) is not None]

            if values:
                if agg_func.upper() == "COUNT":
                    total_count = sum(values)
                    findings.append(f"Total count across all groups: {total_count}")
                    findings.append(f"Average per group: {total_count / len(values):.1f}")
                elif agg_func.upper() in ["SUM", "AVG", "MIN", "MAX"]:
                    max_val = max(values)
                    min_val = min(values)
                    findings.append(f"{agg_func.upper()} of {agg_col}: ranges from {min_val} to {max_val}")

        if len(results) > 1:
            findings.append(f"Groups range from '{results[0].get(group_by)}' to '{results[-1].get(group_by)}'")

        recommendations.append("Use filter_data to drill down into specific groups")
        recommendations.append("Consider visualizing this data with export_query_results")

        return QueryInsight(
            summary=f"Aggregation by {group_by} shows {len(results)} distinct groups",
            key_findings=findings,
            data_quality_notes=[],
            recommendations=recommendations,
            raw_data=results,
    ...
    @staticmethod
    def analyze_column_stats(stats: Dict, column: str) -> QueryInsight:
        findings = []
        quality_notes = []
        recommendations = []
...
# ---------------------------------------------------------------------
# MCP Tools (full implementations)
# ---------------------------------------------------------------------

@mcp.tool(name="get_schema", description="Retrieve the database schema for SAP HANA with intelligent analysis")
...
@mcp.tool(name="get_columns", description="Get column names with intelligent metadata")
...
@mcp.tool(name="get_data", description="Retrieve and analyze sample data from the default HANA table")
async def get_data(
    columns: str = Query(None, description="Comma-separated list of columns to retrieve"),
...
@mcp.tool(name="get_table_summary", description="Get comprehensive table summary with insights")
...
@mcp.tool(name="run_sql", description="Run custom SQL with intelligent result analysis")
async def run_sql(query: str = Query(..., description="A safe SELECT SQL query to run")):
    ...
    for col in potential_columns:
        if col.upper() not in VALID_COLUMNS:
            suggestion = get_close_matches(col.upper(), VALID_COLUMNS, n=1)
            if suggestion:
                raise HTTPException(status_code=400, detail=f"Invalid column name: '{col}'. Did you mean '{suggestion[0]}'?")
            else:
...
@mcp.tool(name="search_data", description="Smart search with intelligent result analysis")
async def search_data(
    search_term: str = Query(..., description="Term to search for across text columns"),
    columns: str = Query(None, description="Specific columns to search in (comma-separated)"),
    ...
    response["raw_data"]["search_metadata"] = {"search_term": search_term, "columns_searched": text_columns, "case_sensitive": case_sensitive}

    return response


@mcp.tool(name="filter_data", description="Filter data with intelligent result analysis")
async def filter_data(
    filters: Dict[str, Any],  # Accept dict directly
    ...
    cursor = conn.cursor()
    try:
        filter_dict = filters  # Already a dict
...
@mcp.tool(name="filter_by_date_range", description="Date-based filtering with intelligent analysis")
async def filter_by_date_range(
    date_column: str = Query(..., description="Date or timestamp column to filter"),
    start_date: str = Query(..., description="Start date (e.g., 2023-01-01)"),
    end_date: str = Query(..., description="End date (e.g., 2023-12-31)"),
    date_format: str = Query("YYYY-MM-DD", description="Date format used in start and end date"),
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Invalid date or format. Check 'date_format' and try again.")
...
@mcp.tool(name="aggregate_data", description="Perform aggregations with intelligent insights")
async def aggregate_data(
    group_by: str = Query(..., description="Column to group by"),
    aggregations: Dict[str, str] = Query(..., description="JSON/dict of aggregations like {'column': 'function'}"),
    having_filter: Optional[str] = Query(None, description="HAVING clause filter"),
    limit: int = Query(100, description="Maximum groups to return"),
):
    cursor = conn.cursor()
    try:
        agg_dict = aggregations
        ...
        for column, function in agg_dict.items():
            ...
            if func not in valid_functions:
                raise HTTPException(status_code=400, detail=f"Invalid function: '{function}'. Use: {valid_functions}")
    ...
    # Build findings like filter_data
...
@mcp.tool(name="get_distinct_values", description="Analyze unique values with intelligent insights")
async def get_distinct_values(
    column: str = Query(..., description="Column name to get distinct values for"),
    limit: int = Query(50, description="Maximum number of distinct values to return"),
    search_filter: str = Query(None, description="Filter distinct values containing this term"),
):
    validate_column(column)
...
@mcp.tool(name="get_data_types", description="Comprehensive column metadata with recommendations")
...
@mcp.tool(name="export_query_results", description="Export with format-specific insights")
async def export_query_results(
    query: str = Query(None, description="SELECT query to export"),
    format: str = Query("csv", description="Export format: json, csv, or summary"),
    include_headers: bool = Query(True, description="Include column headers"),
):
    """Export table data with intelligent analysis."""
